...
RowsGroup.prototype = {
    ...
    _stripeRows: function () {
        var columnsIndexes = this.table.columns(this.columnsForGrouping, ShowedDataSelectorModifier).indexes().toArray()
        var showedRowsCount = this.table.rows(ShowedDataSelectorModifier)[0].length
		this._stripeColumns(0, showedRowsCount - 1, columnsIndexes);
    },

	_stripeColumns(iStartRow, iFinishRow, columnsIndexes) {
		if (columnsIndexes.length < 1) return;

        var columnsIndexesCopy = columnsIndexes.slice()
		currentColumn = columnsIndexesCopy.shift()
        currentColumn = this.table.column(currentColumn, ShowedDataSelectorModifier)
        var columnNodes = currentColumn.nodes()
		if ($(columnNodes[iStartRow]).parent().length > 0) {
			for (iRow = iStartRow; iRow <= iFinishRow; iRow++) {
                var row = $(columnNodes[iRow]).parent();
				this._removeStripedClasses(row);

				if (row.children().first().is(":visible")) {
                    this._switchCurrentStripe();
                    this._setStripedClass(row, "start_group", iRow);
                } else {
					this._setStripedClass(row, "continue_group", iRow);
                }
            }
		} else {
			 this._stripeColumns(iStartRow, iFinishRow, columnsIndexesCopy);
        }
    },

    curStripeClass: "odd",
    _setStripedClass: function(row, rowType, rowIndex) {
		row.addClass(this.curStripeClass);
		row.addClass(rowType);
		row.addClass(`row-index-${rowIndex}`)
    },

	_switchCurrentStripe: function() {
		this.curStripeClass = this.curStripeClass === "odd" ? "even" : "odd";
	},

	_removeStripedClasses(row) {
        row.removeClass("odd");
        row.removeClass("even");
		row.removeClass("start_group");
		row.removeClass("continue_group");
    },
