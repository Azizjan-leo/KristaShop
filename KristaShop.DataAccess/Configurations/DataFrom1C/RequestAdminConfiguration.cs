...
namespace KristaShop.DataAccess.Configurations.DataFrom1C {
    public class RequestAdminConfiguration : IEntityTypeConfiguration<RequestAdmin> {
        public void Configure(EntityTypeBuilder<RequestAdmin> builder) {
            builder.ToView("requests_admin");
            builder.ToTable("RequestAdmin", t => t.ExcludeFromMigrations());
            builder.HasNoKey();
